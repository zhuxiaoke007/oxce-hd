...
{
	_timer->think(0, this);
	// HD overlay is cleared per frame; keep re-drawing while visible so the
	// embedded text stays stamped (draw() sets its HD origin).
...
void WarningMessage::draw()
{
	...
	// Embedded text is blitted onto this surface (relative coords): give it
	// this widget's absolute origin for the HD overlay path.
	_text->setHdOrigin(getX(), getY());
