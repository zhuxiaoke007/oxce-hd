 ...
 */
void ActionMenuItem::think()
{
	InteractiveSurface::think();
	// The HD overlay is cleared every frame; embedded texts are only
	// re-stamped from draw(), so keep this item in a redraw state.
	if (Options::hdFonts && !Options::useOpenGL)
		setRedraw(true);
}
...
void ActionMenuItem::draw()
{
	...
	// Embedded texts (blitted onto this item surface, relative coords): the
	// HD overlay path needs this item's absolute origin, and texts must
	// re-draw every frame so glyphs stay stamped in the per-frame-cleared
	// overlay (same treatment as TextButton labels).
	_txtDescription->setRedraw(true);
	_txtDescription->setHdOrigin(getX(), getY());
	_txtAcc->setRedraw(true);
	_txtAcc->setHdOrigin(getX(), getY());
	_txtTU->setRedraw(true);
	_txtTU->setHdOrigin(getX(), getY());
