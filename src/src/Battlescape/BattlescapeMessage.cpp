...
void BattlescapeMessage::blit(SDL_Surface *surface)
{
	...
	// Embedded texts use absolute coords here (blit target = screen buffer);
	// they must re-draw every frame to stay stamped in the HD overlay.
	_text->setRedraw(true);
	_txtThinking->setRedraw(true);
	_text->blit(surface);
