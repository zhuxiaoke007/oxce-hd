...
OptionsAdvancedState::OptionsAdvancedState(OptionsOrigin origin) : OptionsBaseState(origin)
{
	...
	_edtString = new TextEdit(this, 196, 9, 0, 0);
	_editingString = 0;
	...
	_edtString->onKeyboardPress((ActionHandler)&OptionsAdvancedState::edtStringKeyPress);
...
void OptionsAdvancedState::addSettings(const std::vector<OptionInfo> &settings)
{
	...
	const size_t total = settings.size();
	for (size_t idx = 0; idx < total; ++idx)
	{
		const auto& optionInfo = settings[idx];
		...
		// OPTION_STRING options are not surfaced in the Advanced list
		// (edited via a dedicated GUI); skip them defensively.
		else
		{
			continue;
		}
		Log(LOG_INFO) << "addSettings [" << (idx + 1) << "/" << total
		              << "] BEFORE addRow  id=\"" << optionInfo.id()
		              << "\" name=\"" << name << "\" (bytes=" << name.size()
		              << ") value=\"" << value << "\" (bytes=" << value.size() << ")";
		_lstOptions->addRow(2, name.c_str(), value.c_str());
		Log(LOG_INFO) << "addSettings [" << (idx + 1) << "/" << total
		              << "] AFTER  addRow  id=\"" << optionInfo.id() << "\"";
...
void OptionsAdvancedState::lstOptionsClick(Action *action)
{
	...
	if (setting->type() == OPTION_STRING) // string variables are edited inline
	{
		// commit/cancel any edit in progress before starting a new one
		if (_edtString->isFocused())
		{
			stopStringEdit();
		}
		_editingString = setting->asString();
		_edtString->setText(*_editingString);
		_edtString->setX(_lstOptions->getColumnX(0));
		_edtString->setY(_lstOptions->getRowY(sel));
		_edtString->setVisible(true);
		_edtString->setFocus(true, false);
		_lstOptions->setScrolling(false);
		return; // value text is refreshed when the edit is committed
	}
	if (setting->type() == OPTION_BOOL)
	{
		...
		*b = !*b;
		...
		if (b == &Options::hdFonts)
		{
			Options::reload = true; // reload mods to attach/detach the HD font backends
		}
	}
 ...
 */
void OptionsAdvancedState::stopStringEdit()
{
	_edtString->setText("");
	_edtString->setVisible(false);
	_edtString->setFocus(false, false);
	_lstOptions->setScrolling(true);
	_editingString = 0;
}

/**
 * Commits or cancels the inline edit of a string option.
 * @param action Pointer to an action.
 */
void OptionsAdvancedState::edtStringKeyPress(Action *action)
{
	if (!_editingString)
	{
		return;
		...
		action->getDetails()->key.keysym.sym == SDLK_KP_ENTER)
	{
		...
		std::string *target = _editingString;
		*target = _edtString->getText();
		stopStringEdit();
		// font resource changes need a mod reload to take effect
		if (target == &Options::hdFontTtf || target == &Options::hdFontTtfGeo || target == &Options::hdFontTtfCJK)
		{
			Options::reload = true;
		}
		updateList();
