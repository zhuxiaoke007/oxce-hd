...
{
	_timer->think(0, this);
	// HD overlay is cleared per frame; keep re-drawing so facility labels
	// stay stamped (draw() sets their HD origin).
...
void BaseView::draw()
{
	...
	for (auto* fac : *_base->getFacilities())
	{
		...
		if (fac->getBuildTime() > 0 || fac->getDisabled())
		{
			...
			text->setText(ss.str());
			// Temporary embedded text: give it this view's absolute origin
			// for the HD overlay path.
