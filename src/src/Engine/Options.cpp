...
void createOptionsOXC()
{
	...
	_info.push_back(OptionInfo(OPTION_OXC, "hdFonts", &hdFonts, false, "STR_HD_FONTS", "STR_GENERAL")); // HD font backend (display-resolution TTF text overlay)
	_info.push_back(OptionInfo(OPTION_OXC, "hdFontScale", &hdFontScale, 100, "STR_HD_FONT_SCALE", "STR_GENERAL")); // HD font size scale, percent (100/110/125 recommended)
	_info.push_back(OptionInfo(OPTION_OXC, "hdFontTtf", &hdFontTtf, "")); // primary TTF (UI fonts). Empty = built-in default. (Not in Advanced list: edited via dedicated GUI.)
	_info.push_back(OptionInfo(OPTION_OXC, "hdFontTtfGeo", &hdFontTtfGeo, "")); // TTF for FONT_GEO_* (monospace digits). Empty = built-in default.
	_info.push_back(OptionInfo(OPTION_OXC, "hdFontTtfCJK", &hdFontTtfCJK, "")); // CJK fallback TTF. Empty = built-in default.
