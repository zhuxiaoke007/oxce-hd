...
void Zoom::flipWithZoom(SDL_Surface *src, SDL_Surface *dst, int topBlackBand, int bottomBlackBand, int leftBlackBand, int rightBlackBand, OpenGL *glOut)
{
	...
	{
		SDL_Surface *tmp = SDL_CreateRGBSurface(0, dstWidth, dstHeight, dst->format->BitsPerPixel, 0, 0, 0, 0);
		...
		// Keep the vanilla SDL blit: the upscale's palette translation is
		// relied upon by the game's bitmap rendering (it maps buffer indices
		// onto the screen palette, e.g. battle stat digits); a plain index
		// copy would render those at raw indices that can be black in the
		// palette. The HD overlay composite must not corrupt the palette
		// (see Screen::flip).
