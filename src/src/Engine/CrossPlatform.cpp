...
// On Windows we can use SEH (and dbghelp) for a real stack trace at the
// crash site. This works for MSVC as well as MinGW-w64 64-bit (SEH) and
// 32-bit (SJLJ/DWARF) as long as an exception filter is installed in main.
#if defined(_MSC_VER) || (defined(_WIN32) && !defined(__NO_DBGHELP))
