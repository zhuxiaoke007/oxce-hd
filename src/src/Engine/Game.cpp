	...
	while (!_quit)
	{
		...
		if (runningState != PAUSED)
		{
			...
			if (_init && _timeUntilNextFrame <= 0)
			{
				...
				{
#ifdef __HDFONTS__
					// The HD text overlay is composited above the whole frame,
					// so text of states occluded by the states above them must
					// NOT go there (it would bleed through popup windows).
					// Only the topmost blitted state may stamp HD text; lower
					// ones fall back to the bitmap path, which the normal blit
					// order occludes correctly.
					if (Options::hdFonts && !Options::useOpenGL)
						_screen->setHdOverlayEnabled(std::next(i) == _states.end());
#endif
					(*i)->blit();
				}
