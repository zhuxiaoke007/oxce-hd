...
{
#ifdef __HDFONTS__
	// The HD text overlay is wiped every frame (Screen::clear), but surfaces
	// normally draw once and cache. Text-bearing surfaces of the TOPMOST
	// blitted state (the only one whose overlay is enabled, see Game) must
	// re-draw every frame so glyphs are re-stamped into the overlay; glyph
	// raster results are cached by the font, so this is a cheap blit.
	const bool hdPerFrame = Options::hdFonts && !Options::useOpenGL
		&& _game->getScreen()->getTextOverlaySurface() != 0;
#endif
	for (auto* surface : _surfaces)
	{
#ifdef __HDFONTS__
		if (hdPerFrame &&
			(dynamic_cast<Text*>(surface) || dynamic_cast<TextButton*>(surface) ||
			 dynamic_cast<TextEdit*>(surface) || dynamic_cast<TextList*>(surface) ||
			 dynamic_cast<ComboBox*>(surface)))
		{
			surface->setRedraw(true);
		}
#endif
