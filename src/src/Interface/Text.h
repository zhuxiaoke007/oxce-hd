...
private:
	...
	int _scrollY;
	/// When true, this text is rendered into a parent surface (relative coords)
	/// and must NOT use the HD overlay path, which assumes absolute positioning.
	bool _hdEnabled;
	/// Base-resolution origin of the owning surface for embedded texts.
	/// The HD overlay draws at absolute screen coordinates, so the parent
	/// surface's position must be added to this text's relative position.
	int _hdOriginX, _hdOriginY;
...
public:
	...
	/// Enables/disables the HD overlay path for this text.
	void setHdEnabled(bool enabled);
	/// Sets the base-resolution origin of the owning surface (embedded texts).
	void setHdOrigin(int x, int y);
