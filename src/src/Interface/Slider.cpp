...
void Slider::blit(SDL_Surface *surface)
{
	...
	if (_visible && !_hidden)
	{
		// Embedded texts use absolute coords here (blit target = screen
		// buffer); re-draw every frame to stay in the HD overlay.
		_txtMinus->setRedraw(true);
		_txtPlus->setRedraw(true);
