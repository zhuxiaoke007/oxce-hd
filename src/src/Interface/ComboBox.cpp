...
void ComboBox::blit(SDL_Surface *surface)
{
	...
	if (_visible && !_hidden)
	{
		// The embedded button (label) and dropdown list are private members,
		// so State's per-frame redraw pass never reaches them — keep them
		// redrawing so their text stays stamped in the HD overlay (which is
		// cleared every frame). Both use absolute coordinates when blitted
		// here, so their HD origins are correct as-is.
		_button->setRedraw(true);
