...
void TextList::draw()
{
	Surface::draw();
	{
		...
		{
			int cells = 0;
			for (auto &v : _texts) cells += v.size();
			int ink0 = -1;
			std::string t0;
			if (!_texts.empty() && !_texts[0].empty())
			{
				Text *c0 = _texts[0][0];
				t0 = c0->getText();
				SDL_Surface *s = c0->getSurface();
				ink0 = 0;
				if (SDL_LockSurface(s) == 0)
				{
					for (int yy = 0; yy < s->h; ++yy)
						for (int xx = 0; xx < s->w; ++xx)
							if (((const Uint8*)s->pixels)[yy * s->pitch + xx]) ++ink0;
					SDL_UnlockSurface(s);
				}
			}
			Log(LOG_INFO) << "HD-DIAG-LIST: this=" << ((void*)this) << " pos=" << getX() << "," << getY()
				<< " rows=" << _rows.size() << " texts=" << _texts.size() << " cells=" << cells
				<< " visibleRows=" << _visibleRows << " scroll=" << _scroll
				<< " cell0ink=" << ink0 << " cell0text='" << t0.substr(0, 24) << "' draw#" << n;
		}
	}
	int y = 0;
	if (!_rows.empty())
	{
		...
		for (size_t i = _rows[_scroll]; i < _texts.size() && i < _rows[_scroll] + _visibleRows; ++i)
		{
			...
			{
				text->setY(y);
				// Embedded cells must re-draw every frame in HD mode so their
				// glyphs are re-stamped into the (per-frame cleared) overlay.
				text->setRedraw(true);
				// Embedded cells are blitted onto this list surface (relative
				// coords); the HD overlay path needs the list's absolute origin.
				text->setHdOrigin(getX(), getY());
