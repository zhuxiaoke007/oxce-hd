...
TextButton::TextButton(int width, int height, int x, int y) : InteractiveSurface(width, height, x, y), _color(0), _group(0), _contrast(false), _geoscapeButton(false), _comboBox(0)
{
	...
	// The label is blitted onto the button surface (relative coords); the HD
	// overlay path receives the button's absolute position via setHdOrigin().
}
...
void TextButton::draw()
{
	...
	_text->setRedraw(true); // re-stamp into the per-frame-cleared HD overlay
