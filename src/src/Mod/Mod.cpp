...
{
/**
 * Resolves a font spec into an openable path. A spec may carry a trailing
 * "#<index>" to select a face inside a TTC collection; that suffix is
 * stripped while probing for the file and re-appended to the result so the
 * downstream TTF_OpenFontIndex call receives a clean path + index.
 * If `spec` already exists as a file it is returned unchanged; otherwise it
 * is treated as a bare filename and searched in the standard Windows / MSYS2
 * font directories. If nothing is found the original spec is returned so
 * TTF_OpenFont can report a clear error and the engine falls back to bitmap.
 */
std::string resolveHdFontPath(const std::string &spec)
{
	if (spec.empty())
		return spec;
	// Separate the optional "#index" TTC face selector.
	std::string base = spec, suffix;
	std::string::size_type hash = spec.find('#');
	if (hash != std::string::npos)
	{
		base = spec.substr(0, hash);
		suffix = spec.substr(hash); // includes the '#'
	}
	// Already an existing file (absolute or relative to cwd)?
	SDL_RWops *rw = SDL_RWFromFile(base.c_str(), "rb");
	if (rw) { SDL_RWclose(rw); return spec; }
	// Search the configured data folder first, so bundled fonts shipped with
	// the game resolve regardless of the process working directory (hermetic
	// "HD fonts"). Uses 'base' so "fonts/foo.ttf" maps to "<data>/fonts/foo.ttf".
	if (!Options::getDataFolder().empty())
	{
		std::string p = Options::getDataFolder() + base;
		rw = SDL_RWFromFile(p.c_str(), "rb");
		if (rw) { SDL_RWclose(rw); return p + suffix; }
	}
	const char *dirs[] = {
		"C:/Windows/Fonts/", "C:/Windows/System32/Fonts/",
		"/mingw64/share/fonts/", "/usr/share/fonts/", 0
	};
	for (int i = 0; dirs[i]; ++i)
	{
		std::string p = std::string(dirs[i]) + base;
		rw = SDL_RWFromFile(p.c_str(), "rb");
		if (rw) { SDL_RWclose(rw); return p + suffix; }
	}
	return spec;
...
void Mod::loadExtraResources()
{
	...
	for (const auto& fontReader : reader["fonts"].children())
	{
		...
		font->setId(id);
		// Optional HD (TTF) font backend: pick a role-appropriate TTF and a
		// CJK fallback, then size automatically from the bitmap cell height.
		if (Options::hdFonts)
		{
			std::string primary, cjk;
			// Geo fonts (globe/funding digits) use a monospaced face so digit
			// columns stay aligned; everything else uses a clean sans.
			bool isGeo = id.rfind("FONT_GEO", 0) == 0;
			// Default HD faces must be SMOOTH vector outlines: the whole point
			// of the feature is crisp strokes instead of chunky bitmap pixels.
			// Prefer smooth system faces, then a bundled smooth face; the
			// bundled Ark Pixel face is only a last-resort fallback.
			if (!Options::hdFontTtf.empty())
			{
				primary = resolveHdFontPath(Options::hdFontTtf);
			}
			else if (isGeo && !Options::hdFontTtfGeo.empty())
			{
				primary = resolveHdFontPath(Options::hdFontTtfGeo);
			}
			else
			{
				const char *uiFaces[] = { "arial.ttf", "DejaVuSans.ttf", 0 };
				const char *geoFaces[] = { "consola.ttf", "DejaVuSansMono.ttf", 0 };
				const char **faces = isGeo ? geoFaces : uiFaces;
				for (int i = 0; faces[i]; ++i)
				{
					std::string p = resolveHdFontPath(faces[i]);
					if (p != faces[i]) { primary = p; break; } // resolved to an existing file
				}
				if (primary.empty())
					primary = resolveHdFontPath("fonts/ark-pixel-16px-monospaced-latin.ttf");
			}

			if (!Options::hdFontTtfCJK.empty())
			{
				cjk = resolveHdFontPath(Options::hdFontTtfCJK);
			}
			else
			{
				// Smooth CJK faces first (Microsoft YaHei / SimHei on Windows),
				// bundled WenQuanYi Zen Hei (wqy-16px.ttf, GPL with font
				// embedding exception) as the hermetic fallback.
				const char *cjkFaces[] = { "msyh.ttc#0", "msyh.ttf", "simhei.ttf", 0 };
				for (int i = 0; cjkFaces[i]; ++i)
				{
					std::string p = resolveHdFontPath(cjkFaces[i]);
					if (p != cjkFaces[i]) { cjk = p; break; }
				}
				if (cjk.empty())
					cjk = resolveHdFontPath("fonts/wqy-16px.ttf");
			}

			font->loadHdFont(primary, cjk);
		}
		_fonts[id] = font;
	}
