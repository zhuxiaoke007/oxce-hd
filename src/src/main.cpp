...
#if defined(_MSC_VER) || defined(__MINGW64__) || defined(__MINGW32__) || defined(__x86_64__)
...
int main(int argc, char *argv[])
{
	...
	// MinGW-w64 64-bit defaults to SEH exceptions, so we can install an
	// unhandled-exception filter that captures a real stack trace at the
	// crash site (instead of the no-op signal handler below, which only sees
	// its own frame). 32-bit MinGW still uses SJLJ/DWARF and cannot.
	SetUnhandledExceptionFilter(crashLogger);
	std::set_terminate(exceptionLogger);
#else
...
#ifdef __HDFONTS__
	// Temporary P5 validation hook: -hdshot:<savefile> auto-loads the save,
	// screenshots HD + legacy rendering of the same scene, then quits.
	{
		const auto &args = CrossPlatform::getArgs();
		Log(LOG_INFO) << "HDTEST: argc=" << args.size();
		for (size_t i = 0; i < args.size(); ++i)
		{
			Log(LOG_INFO) << "HDTEST: arg[" << i << "]=" << args[i];
		}
		for (size_t i = 0; i < args.size(); ++i)
		{
			if (args[i].compare(0, 8, "-hdshot:") == 0)
			{
				Log(LOG_INFO) << "HDTEST: hook armed for save=" << args[i].substr(8);
				// Load all game data synchronously (StartState::load would do
				// this in a background thread before showing the main menu).
				Options::updateMods();
				game->loadMods();
				game->loadLanguages();
				Log(LOG_INFO) << "HDTEST: data loaded, entering hook flow";
				game->setState(new HdShotLoader(args[i].substr(8)));
				break;
			}
		}
	}
