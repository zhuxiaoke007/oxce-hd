...
| SHA-256 | `A1EE313F…`（2026-08-25 重建：P10 Win32 IME 桥（中文输入法）+ GEO 时间按钮字号统一/数字居中，见 `P10_Win32_Ime_Bridge_Report.md`） |
| 大小 | 23,242,894 字节 |
| 修改时间 | 2026-08-25（P10 修复构建） |
...
> - `42C05741...`（2026-08-24）：P8 非整数缩放修复——内容缩放全链路 int→double（Screen::baseToDisplay 按真实分数缩放映射、Font TTF 字号/字形格按真实比例取整）。
> - `EB5A9435...`（2026-08-25）：P9 弹出窗口遮挡修复——Window::blit 覆写 + Screen::clearTextOverlayRect（窗口背景绘制前清掉覆盖层矩形墨迹，下拉菜单不再有背景正文穿透）。
> - `A1EE313F...`（2026-08-25 最新）：P10——Win32 IME 桥（WM_IME_CHAR/COMPOSITION → SDL_KEYDOWN，中文输入法可在 TextEdit 输入）；地球层时间按钮字号与侧栏统一（移除 setBig）、CJK 面数字居中放置使字隙均匀；Geo 字体内含汉字行数字改走 CJK 面（行级检测）。
